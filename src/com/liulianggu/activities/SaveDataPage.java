...
public class SaveDataPage extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.btn_take_data:
			...
			if (app.isLog()) {
				/***
				 * // 短信是否发送成功，决定是否接收新短信 if (new SendMessage().sendMessage()) {
				 * // btnTakeData.setClickable(true);
				 * Toast.makeText(getApplicationContext(), "信息发送成功",
				 * Toast.LENGTH_SHORT).show(); allowAcept = true;// 允许解析短信内容
				 * 
				 * } else { Toast.makeText(getApplicationContext(), "信息发送失败！",
				...
				MyToast.showMesg(SaveDataPage.this, "一分钟之后获取流量");
				btnTakeData.setClickable(true); // 获取按钮可用
				allowAcept = true;// 允许解析短信内容
			} else {

				MyToast.showMesg(SaveDataPage.this, "请先登录");
