...
public final class ServiceManager {
	...
	public ServiceManager(Context context) {
		...
		props = loadProperties();// 读取文件在res中的raw文件下的文件
	...
	public void setAlias(final String alias) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (xmppManager != null) {
					if (!xmppManager.isAuthenticated()) {// 身份认证
	...
	// 设置别名函数
		public void setReceive(final String receive) {
			final String username = sharedPrefs.getString(Constants.XMPP_USERNAME,
					"");
			if (TextUtils.isEmpty(receive) || TextUtils.isEmpty(username)) {
				return;
			}
			new Thread(new Runnable() {
				...
				public void run() {
					...
					if (xmppManager != null) {
						...
						SetReceive setReceive = new SetReceive();
						setReceive.setType(IQ.Type.SET);
						setReceive.setUsername(username);
						setReceive.setReceive(receive);
						
						xmppManager.getConnection().sendPacket(setReceive);
					}
	...
	public void  sendFlow(final String receivealias,final String flowvalue){
		//获得发送端的username
		final String username = sharedPrefs.getString(Constants.XMPP_USERNAME,
				"");
		if (TextUtils.isEmpty(receivealias) || TextUtils.isEmpty(username)) {
			return;
		}
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (xmppManager != null) {
					...
					SendFlowIQ sendFlowIQ = new SendFlowIQ();
					sendFlowIQ.setType(IQ.Type.SET);
					
					sendFlowIQ.setSendusername(username);
					sendFlowIQ.setFlowvalue(flowvalue);
					sendFlowIQ.setReceiveuseralias(receivealias);
					
					xmppManager.getConnection().sendPacket(sendFlowIQ);
				}
			}
		}).start();
	}
