...
public class NotificationService extends Service {
    ...
    private SharedPreferences sharedPrefs;//SharedPreferences是Android中最容易理解的数据存储技术，
    									  //实际上SharedPreferences处理的就是一个key-value（键值对）。
    private String deviceId;

    public NotificationService() {
        notificationReceiver = new NotificationReceiver();
        connectivityReceiver = new ConnectivityReceiver(this);//检测网络状态
        phoneStateListener = new PhoneStateChangeListener(this);//检测网络状态
    ...
    public void onCreate() {
        ...
        xmppManager = new XmppManager(this);//启动一个XmppManager

        taskSubmitter.submit(new Runnable() {//该submit方法的主要作用是开启一个线程池
            public void run() {
                NotificationService.this.start();//子线程运行
            }
        });
    }
    ...
    private void start() {
        Log.d(LOGTAG, "start()...");
        registerNotificationReceiver();//注册通知的监听器
        registerConnectivityReceiver();//注册网络连接的监听器
